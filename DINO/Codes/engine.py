...
if __name__ == '__main__':
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_loss_hist.value < BEST_VAL_LOSS:
            ...
            torch.save(model.state_dict(), f"{OUT_DIR}/model_best_loss_rcnn_res.pth")
            print('SAVING MODEL COMPLETE at BEST LOSS of {} at epoch {}...\n'.format(BEST_VAL_LOSS,epoch+1))

        # if (epoch + 1) % SAVE_MODEL_EPOCH == 0:  # save model after every n epochs
        #     torch.save(model.state_dict(), f"{OUT_DIR}/model_rcnn_res{epoch + 1}.pth")
        ...
        figure1.savefig(f"{OUT_DIR}/train_val_loss_rcnn_res.png")
        plt.close('all')
        print('SAVING TRAINING AND VALIDATION LOSS PLOT COMPLETE...')
        # save training and validation loss lists
        with open(f"{OUT_DIR}/train_loss_list_rcnn_res.txt", "w") as file:
            file.write(str(train_list))
        with open(f"{OUT_DIR}/val_loss_list_rcnn_res.txt", "w") as file:
