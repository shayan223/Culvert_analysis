...
def qualitative_validation():
    ...
    VAL_PATH = '/user/DETR/validation_results'
    ...
    trainRCNN = pd.read_csv('/user/DETR/validation_results/val_labels.csv', sep=",")
    ...
    for imageFileName in image_names:
        ...
        for _,row in trainRCNN[trainRCNN.filename == imageFileName].iterrows():
            ...
            if not os.path.exists("/user/DETR/validation_qualitative"):
                os.makedirs("/user/DETR/validation_qualitative")

            # fig.savefig('validation_qualitative/' + os.path.splitext(imageFileName)[0] + ".jpg", dpi=90, bbox_inches='tight')
            fig.savefig('/user/DETR/validation_qualitative/' + os.path.splitext(imageFileName)[0] + ".jpg", dpi=90, bbox_inches='tight')
        plt.close()
